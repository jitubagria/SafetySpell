...
export class AdminTagsService {
  ...
  async create(actor: string, input: { categoryKey: string; form: string; quantity: number }) {
    ...
    return this.db.transaction(async (client) => {
      ...
      const batch = await client.query<{ id: string }>(
        `INSERT INTO tag_batches(batch_code,category_id,form,quantity,created_by_user_id) VALUES($1,$2,$3,$4,$5) RETURNING id`,
  ...
  private async tag(tagCode: string) {
    const result = await this.db.query<{ code: string; category: string; color: string }>(
      `SELECT t.code,c.key AS category,c.color FROM tags t JOIN categories c ON c.id=t.category_id WHERE t.code=$1`,
      [normalizeTagCode(tagCode)],
    );
    if (!result.rows[0]) throw new NotFoundException("Tag unavailable");
    return result.rows[0];
  }
  async png(tagCode: string) {
    const tag = await this.tag(tagCode);
  ...
  async pdf(batchId: string) {
    const rows = await this.db.query<{ code: string; color: string }>(
      `SELECT t.code, c.color FROM tags t JOIN categories c ON c.id=t.category_id
       WHERE t.batch_id=$1 ORDER BY t.code`,
      [batchId],
    );
